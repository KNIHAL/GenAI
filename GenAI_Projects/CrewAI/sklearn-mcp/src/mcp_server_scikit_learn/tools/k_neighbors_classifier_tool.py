"""Tool for training and predicting with scikit-learn's KNeighborsClassifier."""
...
class KNeighborsClassifierInputSchema(BaseModel):
    """Input schema for the K-Neighbors Classifier tool.
    ...
    def from_str(
    ...
    ) -> "KNeighborsClassifierInputSchema":
        ...
        Returns:
            A KNeighborsClassifierInputSchema instance.
        """
        ...
        return KNeighborsClassifierInputSchema(
...
k_neighbors_classifier_tool = types.Tool(
    name=MCPServerScikitLearnTools.K_NEIGHBORS_CLASSIFIER.value,
    description="Train and predict with K-Neighbors Classifier.",
    inputSchema=KNeighborsClassifierInputSchema.input_schema(),
)


async def handle_k_neighbors_classifier_tool(
    arguments: dict[str, Any],
) -> list[types.TextContent | types.ImageContent | types.EmbeddedResource]:
    """Handle requests for the K-Neighbors Classifier tool.
    ...
    """
    logger.info("Handling k-neighbors classifier tool")
    ...
    k_neighbors_classifier_input = KNeighborsClassifierInputSchema.from_str(
        training_dataset,
        target_column,
        prediction_dataset,
        column_to_ignore,
    ...
    train_df = pl.DataFrame(k_neighbors_classifier_input.training_dataset)
    prediction_df = pl.DataFrame(k_neighbors_classifier_input.prediction_dataset)
    training_columns = []
    ...
    for col in train_df.columns:
        if col == k_neighbors_classifier_input.target_column:
            continue
        if k_neighbors_classifier_input.column_to_ignore and col in k_neighbors_classifier_input.column_to_ignore:
            continue
    ...
    x = train_df.drop(k_neighbors_classifier_input.target_column)
    if k_neighbors_classifier_input.column_to_ignore:
        x = x.drop(*k_neighbors_classifier_input.column_to_ignore)
    x = x.to_numpy()
    y = train_df[k_neighbors_classifier_input.target_column].to_numpy()

    # Train the model
    estimator = KNeighborsClassifier(
        n_neighbors=k_neighbors_classifier_input.n_neighbors,
        weights=k_neighbors_classifier_input.weights,
        algorithm=k_neighbors_classifier_input.algorithm,
        leaf_size=k_neighbors_classifier_input.leaf_size,
        p=k_neighbors_classifier_input.p,
        metric=k_neighbors_classifier_input.metric,
        metric_params=k_neighbors_classifier_input.metric_params,
        n_jobs=k_neighbors_classifier_input.n_jobs,
    )
    estimator.fit(x, y)

    # Prepare features for prediction
    prediction_x = (
        prediction_df.drop(k_neighbors_classifier_input.target_column)
        if k_neighbors_classifier_input.target_column in prediction_df.columns
        else prediction_df
    )
    if k_neighbors_classifier_input.column_to_ignore:
        prediction_x = prediction_x.drop(*k_neighbors_classifier_input.column_to_ignore)
    ...
    return [
        types.TextContent(
            ...
            text=json.dumps(
                {
                    "description": "Result of K-Neighbors Classifier.",
