"""Unit tests for the k-neighbors regressor tool."""
...
from mcp_server_scikit_learn.tools.k_neighbors_regressor_tool import (
...
@pytest.mark.parametrize(
    ...
    [
        (
            {
                "training_dataset": [
                    {"feature1": random.randint(1, 10), "feature2": random.randint(1, 20), "target": random.random()}
                    for i in range(20)
                ],
                "prediction_dataset": [
...
async def test_handle_k_neighbors_regressor_tool(
    mocker,
    scope_function,
    input_args: Dict[str, Any],
    expected_keys: list[str],
) -> None:
    """Test the k-neighbors regressor tool handler.
    ...
    # Call the handler
    result = await handle_k_neighbors_regressor_tool(input_args)
